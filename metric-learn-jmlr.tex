...
Its API is compatible with \texttt{scikit-learn} \citep{scikit-learn}, the leading library for machine learning in Python, allowing for streamlined model selection, evaluation, and pipelining with other estimators.
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \caption{(Best seen on computer) Different types of supervision for metric learning. (\subref{fig:full}) Full supervision: make samples from the same label (here, identity) closer together than samples from different labels. (\subref{fig:pairs}) Pair supervision: make similar pairs (here, corresponding to the same person) to be closer than dissimilar pairs (different persons). (\subref{fig:quadruplets}) Quadruplet supervision: make the two leftmost samples (here, people of similar age) to be closer than the two rightmost samples (of bigger age difference). Images are taken from the Labeled Faces in the Wild dataset \citep{Huang12}.}\label{fig:flowers}
\end{figure}
...
those whose input format differs from the classic machine learning setting. It is designed to be fully compatible with the functionalities of \texttt{scikit-learn}. We briefly explain how the package is structured.
...
Weakly supervised algorithms learning from pairs or quadruplets are similar to \texttt{scikit-learn} classifiers, but they \texttt{fit} and \texttt{predict} on a set of tuples. This is passed as a 3D array (rather than a 2D array of points). Tuples can be pairs or quadruplets depending on the algorithm.
...
>>> import sklearn; import metric_learn; import numpy; numpy.random.seed(42)
>>> _, pairs, y, *_ = sklearn.datasets.fetch_lfw_pairs(resize=0.2).values()
>>> pairs, y = sklearn.utils.shuffle((pairs.reshape(2200,2,-1)-127.5)/255, 2*y-1) 
>>> itml = metric_learn.ITML(prior='random')
>>> sklearn.model_selection.cross_val_score(itml, pairs, y, scoring='accuracy')
\end{minted}
% mmc.predict(pairs)
