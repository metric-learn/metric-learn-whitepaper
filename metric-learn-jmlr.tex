...
Other packages for metric-learning already exist, however, they have a different focus: \textit{pyDML} only contains supervised metric learning algorithms, and \textit{pytorch-metric-learning} is focused on deep metric learning, with the pytorch framework. 
\tod[inline]{Talk about the matlab package}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[t]{0.40\textwidth}
        \centering \includegraphics[scale=0.45]{triplets.pdf}
...
This can be used to learn a metric space in which closer points are more similar with respect to an attribute of interest, which may be continuous and difficult to annotate accurately (e.g., the age of a person on an image, see Figure \ref{fig:quadruplets}).
...
Community interest for the package has been demonstrated by its recent inclusion in the \texttt{scikit-learn-contrib} organization which hosts high-quality \texttt{scikit-learn}-compatible projects,\footnote{\url{https://github.com/scikit-learn-contrib/scikit-learn-contrib}} and by its more than 740 stars and 170 forks on GitHub at the time of writing.
...
The package implements one quadruplet learner: Metric Learning from Relative Comparisons by Minimizing Squared Residual \citep[LSML,][]{Liu12}, and one triplet learner: Sparse Compositional Metric Learning \citep[SCML,][]{Shi15}.
...
Weakly supervised algorithms (pair and quadruplet learners) \texttt{fit} and \texttt{predict} on a set of tuples passed as a 3-dimensional array. Tuples can be pairs or quadruplets depending on the algorithm.
